    ...
    def analyze_signal(self):
        ...
        try:
            ...
            if len(Pxx_band) > 0 and np.max(Pxx_band) > 1e-6:
                # IMPROVED: Find prominent peaks to avoid noise
                from scipy.signal import find_peaks
                
                # Find peaks with minimum prominence (25% of max power)
                peaks, properties = find_peaks(Pxx_band, prominence=np.max(Pxx_band) * 0.25)
                
                if len(peaks) > 0:
                    # Use the most prominent peak's frequency
                    most_prominent_idx = peaks[np.argmax(properties['prominences'])]
                    freq_raw = f_band[most_prominent_idx]
                else:
                    # Fallback: use center of mass for smooth estimate
                    freq_raw = np.sum(f_band * Pxx_band) / np.sum(Pxx_band)
                
                # IMPROVED: Temporal smoothing with median filter
                self.freq_history.append(freq_raw)
                freq = float(np.median(self.freq_history))

                # IMPROVED SNR: Peak power vs noise floor in HFOV band
                # This is more clinically relevant than comparing to out-of-band noise
                
                # Method 1: Peak-to-median ratio (good for periodic signals)
                peak_power = np.max(Pxx_band)
                noise_floor = np.median(Pxx_band)  # Use median as noise estimate
                
                if noise_floor <= 0:
                    noise_floor = 1e-12
                
                snr = 10.0 * np.log10(peak_power / noise_floor)
                
                # Alternative Method 2: Compare peak to out-of-band noise
                # Get noise from regions outside HFOV band
                mask_noise = ((f >= 0.5) & (f < 5)) | ((f > 15) & (f < 25))
                if np.sum(mask_noise) > 0:
                    Pxx_noise = Pxx[mask_noise]
                    noise_baseline = np.median(Pxx_noise)
                    if noise_baseline > 0:
                        snr_alt = 10.0 * np.log10(peak_power / noise_baseline)
                        # Use the more conservative (lower) SNR estimate
                        snr = min(snr, snr_alt)
...
class HFOVWindow(QtWidgets.QMainWindow):
    def __init__(self, backend: HFOVBackend):
        ...
        # Vertical line to show detected frequency
        self.freq_marker = self.spec_ax.axvline(x=0, color='#FF5722', linestyle='--', 
                                                 linewidth=3, label='Detected', alpha=0)
    ...
    def update_spectrum(self, force=False):
        ...
        try:
            ...
            # Show detected frequency as vertical line
            detected_freq = self.backend.current_freq
            if detected_freq > 0:
                self.freq_marker.set_xdata([detected_freq, detected_freq])
                self.freq_marker.set_alpha(0.9)  # Make visible
            else:
                self.freq_marker.set_alpha(0)  # Hide if no detection
